...
import com.vertical.commerce.repository.SpecialDealsRepository;
import com.vertical.commerce.service.SpecialDealsService;
import com.vertical.commerce.service.dto.SpecialDealsDTO;
import com.vertical.commerce.service.mapper.SpecialDealsMapper;
import com.vertical.commerce.service.dto.SpecialDealsCriteria;
import com.vertical.commerce.service.SpecialDealsQueryService;
...
public class SpecialDealsResourceIT {
    ...
    public void createSpecialDeals() throws Exception {
        ...
        restSpecialDealsMockMvc.perform(post("/api/special-deals").with(csrf())
    ...
    public void checkLastEditedByIsRequired() throws Exception {
        int databaseSizeBeforeTest = specialDealsRepository.findAll().size();
        // set the field null
        specialDeals.setLastEditedBy(null);
    ...
    public void checkLastEditedWhenIsRequired() throws Exception {
        int databaseSizeBeforeTest = specialDealsRepository.findAll().size();
        // set the field null
        specialDeals.setLastEditedWhen(null);
    ...
    @Test
    @Transactional
    public void getAllSpecialDealsByCartListIsEqualToSomething() throws Exception {
        // Initialize the database
        specialDealsRepository.saveAndFlush(specialDeals);
        ShoppingCarts cartList = ShoppingCartsResourceIT.createEntity(em);
        em.persist(cartList);
        em.flush();
        specialDeals.addCartList(cartList);
        specialDealsRepository.saveAndFlush(specialDeals);
        Long cartListId = cartList.getId();

        // Get all the specialDealsList where cartList equals to cartListId
        defaultSpecialDealsShouldBeFound("cartListId.equals=" + cartListId);

        // Get all the specialDealsList where cartList equals to cartListId + 1
        defaultSpecialDealsShouldNotBeFound("cartListId.equals=" + (cartListId + 1));
    }


    @Test
    @Transactional
    public void getAllSpecialDealsByOrderListIsEqualToSomething() throws Exception {
        // Initialize the database
        specialDealsRepository.saveAndFlush(specialDeals);
        Orders orderList = OrdersResourceIT.createEntity(em);
        em.persist(orderList);
        em.flush();
        specialDeals.addOrderList(orderList);
        specialDealsRepository.saveAndFlush(specialDeals);
        Long orderListId = orderList.getId();

        // Get all the specialDealsList where orderList equals to orderListId
        defaultSpecialDealsShouldBeFound("orderListId.equals=" + orderListId);
    ...
    public void updateSpecialDeals() throws Exception {
        ...
        restSpecialDealsMockMvc.perform(put("/api/special-deals").with(csrf())
            .contentType(MediaType.APPLICATION_JSON)
            .content(TestUtil.convertObjectToJsonBytes(specialDealsDTO)))
            .andExpect(status().isOk());
    ...
    public void deleteSpecialDeals() throws Exception {
        ...
        restSpecialDealsMockMvc.perform(delete("/api/special-deals/{id}", specialDeals.getId()).with(csrf())
