...
import com.vertical.commerce.repository.ProductSetDetailsRepository;
import com.vertical.commerce.service.ProductSetDetailsService;
import com.vertical.commerce.service.dto.ProductSetDetailsDTO;
import com.vertical.commerce.service.mapper.ProductSetDetailsMapper;
import com.vertical.commerce.service.dto.ProductSetDetailsCriteria;
import com.vertical.commerce.service.ProductSetDetailsQueryService;
...
public class ProductSetDetailsResourceIT {
    ...
    public void createProductSetDetails() throws Exception {
        ...
        restProductSetDetailsMockMvc.perform(post("/api/product-set-details").with(csrf())
    ...
    @Test
    @Transactional
    public void getAllProductSetDetailsByProductSetIsEqualToSomething() throws Exception {
        // Initialize the database
        productSetDetailsRepository.saveAndFlush(productSetDetails);
        ProductSet productSet = ProductSetResourceIT.createEntity(em);
        em.persist(productSet);
        em.flush();
        productSetDetails.setProductSet(productSet);
        productSetDetailsRepository.saveAndFlush(productSetDetails);
        Long productSetId = productSet.getId();

        // Get all the productSetDetailsList where productSet equals to productSetId
        defaultProductSetDetailsShouldBeFound("productSetId.equals=" + productSetId);

        // Get all the productSetDetailsList where productSet equals to productSetId + 1
        defaultProductSetDetailsShouldNotBeFound("productSetId.equals=" + (productSetId + 1));
    }


    @Test
    @Transactional
    public void getAllProductSetDetailsByProductIsEqualToSomething() throws Exception {
        // Initialize the database
        productSetDetailsRepository.saveAndFlush(productSetDetails);
        Products product = ProductsResourceIT.createEntity(em);
        em.persist(product);
        em.flush();
        productSetDetails.setProduct(product);
        productSetDetailsRepository.saveAndFlush(productSetDetails);
        Long productId = product.getId();

        // Get all the productSetDetailsList where product equals to productId
        defaultProductSetDetailsShouldBeFound("productId.equals=" + productId);

        // Get all the productSetDetailsList where product equals to productId + 1
        defaultProductSetDetailsShouldNotBeFound("productId.equals=" + (productId + 1));
    }


    @Test
    @Transactional
    public void getAllProductSetDetailsByProductCategoryIsEqualToSomething() throws Exception {
        // Initialize the database
        productSetDetailsRepository.saveAndFlush(productSetDetails);
        ProductCategory productCategory = ProductCategoryResourceIT.createEntity(em);
        em.persist(productCategory);
        em.flush();
        productSetDetails.setProductCategory(productCategory);
        productSetDetailsRepository.saveAndFlush(productSetDetails);
        Long productCategoryId = productCategory.getId();

        // Get all the productSetDetailsList where productCategory equals to productCategoryId
        defaultProductSetDetailsShouldBeFound("productCategoryId.equals=" + productCategoryId);

        // Get all the productSetDetailsList where productCategory equals to productCategoryId + 1
        defaultProductSetDetailsShouldNotBeFound("productCategoryId.equals=" + (productCategoryId + 1));
    }
    ...
    public void updateProductSetDetails() throws Exception {
        ...
        restProductSetDetailsMockMvc.perform(put("/api/product-set-details").with(csrf())
            .contentType(MediaType.APPLICATION_JSON)
            .content(TestUtil.convertObjectToJsonBytes(productSetDetailsDTO)))
            .andExpect(status().isOk());
    ...
    public void deleteProductSetDetails() throws Exception {
        ...
        restProductSetDetailsMockMvc.perform(delete("/api/product-set-details/{id}", productSetDetails.getId()).with(csrf())
