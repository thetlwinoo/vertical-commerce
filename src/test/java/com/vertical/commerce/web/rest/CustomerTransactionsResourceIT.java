...
import com.vertical.commerce.service.dto.CustomerTransactionsCriteria;
import com.vertical.commerce.service.CustomerTransactionsQueryService;
...
public class CustomerTransactionsResourceIT {
    ...
    public void createCustomerTransactions() throws Exception {
        ...
        restCustomerTransactionsMockMvc.perform(post("/api/customer-transactions").with(csrf())
    ...
    public void checkLastEditedByIsRequired() throws Exception {
        int databaseSizeBeforeTest = customerTransactionsRepository.findAll().size();
        // set the field null
        customerTransactions.setLastEditedBy(null);
    ...
    public void checkLastEditedWhenIsRequired() throws Exception {
        int databaseSizeBeforeTest = customerTransactionsRepository.findAll().size();
        // set the field null
        customerTransactions.setLastEditedWhen(null);
    ...
    @Test
    @Transactional
    public void getAllCustomerTransactionsByPaymentMethodIsEqualToSomething() throws Exception {
        // Initialize the database
        customerTransactionsRepository.saveAndFlush(customerTransactions);
        PaymentMethods paymentMethod = PaymentMethodsResourceIT.createEntity(em);
        em.persist(paymentMethod);
        em.flush();
        customerTransactions.setPaymentMethod(paymentMethod);
        customerTransactionsRepository.saveAndFlush(customerTransactions);
        Long paymentMethodId = paymentMethod.getId();

        // Get all the customerTransactionsList where paymentMethod equals to paymentMethodId
        defaultCustomerTransactionsShouldBeFound("paymentMethodId.equals=" + paymentMethodId);

        // Get all the customerTransactionsList where paymentMethod equals to paymentMethodId + 1
        defaultCustomerTransactionsShouldNotBeFound("paymentMethodId.equals=" + (paymentMethodId + 1));
    }
    ...
    @Test
    @Transactional
    public void getAllCustomerTransactionsByOrderIsEqualToSomething() throws Exception {
        // Initialize the database
        customerTransactionsRepository.saveAndFlush(customerTransactions);
        Orders order = OrdersResourceIT.createEntity(em);
        em.persist(order);
        em.flush();
        customerTransactions.setOrder(order);
        customerTransactionsRepository.saveAndFlush(customerTransactions);
        Long orderId = order.getId();

        // Get all the customerTransactionsList where order equals to orderId
        defaultCustomerTransactionsShouldBeFound("orderId.equals=" + orderId);

        // Get all the customerTransactionsList where order equals to orderId + 1
        defaultCustomerTransactionsShouldNotBeFound("orderId.equals=" + (orderId + 1));
    }


    @Test
    @Transactional
    public void getAllCustomerTransactionsByCustomerPaymentIsEqualToSomething() throws Exception {
        // Initialize the database
        customerTransactionsRepository.saveAndFlush(customerTransactions);
        CustomerPayment customerPayment = CustomerPaymentResourceIT.createEntity(em);
        em.persist(customerPayment);
        em.flush();
        customerTransactions.setCustomerPayment(customerPayment);
        customerPayment.setCustomerTransaction(customerTransactions);
        customerTransactionsRepository.saveAndFlush(customerTransactions);
        Long customerPaymentId = customerPayment.getId();

        // Get all the customerTransactionsList where customerPayment equals to customerPaymentId
        defaultCustomerTransactionsShouldBeFound("customerPaymentId.equals=" + customerPaymentId);

        // Get all the customerTransactionsList where customerPayment equals to customerPaymentId + 1
        defaultCustomerTransactionsShouldNotBeFound("customerPaymentId.equals=" + (customerPaymentId + 1));
    }
    ...
    public void updateCustomerTransactions() throws Exception {
        ...
        restCustomerTransactionsMockMvc.perform(put("/api/customer-transactions").with(csrf())
            .contentType(MediaType.APPLICATION_JSON)
            .content(TestUtil.convertObjectToJsonBytes(customerTransactionsDTO)))
            .andExpect(status().isOk());
    ...
    public void deleteCustomerTransactions() throws Exception {
        ...
        restCustomerTransactionsMockMvc.perform(delete("/api/customer-transactions/{id}", customerTransactions.getId()).with(csrf())
