...
import com.vertical.commerce.repository.SupplierTransactionsRepository;
import com.vertical.commerce.service.SupplierTransactionsService;
import com.vertical.commerce.service.dto.SupplierTransactionsDTO;
import com.vertical.commerce.service.mapper.SupplierTransactionsMapper;
import com.vertical.commerce.service.dto.SupplierTransactionsCriteria;
import com.vertical.commerce.service.SupplierTransactionsQueryService;
...
public class SupplierTransactionsResourceIT {
    ...
    public void createSupplierTransactions() throws Exception {
        ...
        restSupplierTransactionsMockMvc.perform(post("/api/supplier-transactions").with(csrf())
    ...
    public void checkLastEditedByIsRequired() throws Exception {
        int databaseSizeBeforeTest = supplierTransactionsRepository.findAll().size();
        // set the field null
        supplierTransactions.setLastEditedBy(null);
    ...
    public void checkLastEditedWhenIsRequired() throws Exception {
        int databaseSizeBeforeTest = supplierTransactionsRepository.findAll().size();
        // set the field null
        supplierTransactions.setLastEditedWhen(null);
    ...
    @Test
    @Transactional
    public void getAllSupplierTransactionsByOrderIsEqualToSomething() throws Exception {
        // Initialize the database
        supplierTransactionsRepository.saveAndFlush(supplierTransactions);
        Orders order = OrdersResourceIT.createEntity(em);
        em.persist(order);
        em.flush();
        supplierTransactions.setOrder(order);
        supplierTransactionsRepository.saveAndFlush(supplierTransactions);
        Long orderId = order.getId();

        // Get all the supplierTransactionsList where order equals to orderId
        defaultSupplierTransactionsShouldBeFound("orderId.equals=" + orderId);

        // Get all the supplierTransactionsList where order equals to orderId + 1
        defaultSupplierTransactionsShouldNotBeFound("orderId.equals=" + (orderId + 1));
    }


    @Test
    @Transactional
    public void getAllSupplierTransactionsByInvoiceIsEqualToSomething() throws Exception {
        // Initialize the database
        supplierTransactionsRepository.saveAndFlush(supplierTransactions);
        Invoices invoice = InvoicesResourceIT.createEntity(em);
        em.persist(invoice);
        em.flush();
        supplierTransactions.setInvoice(invoice);
        supplierTransactionsRepository.saveAndFlush(supplierTransactions);
        Long invoiceId = invoice.getId();

        // Get all the supplierTransactionsList where invoice equals to invoiceId
        defaultSupplierTransactionsShouldBeFound("invoiceId.equals=" + invoiceId);

        // Get all the supplierTransactionsList where invoice equals to invoiceId + 1
        defaultSupplierTransactionsShouldNotBeFound("invoiceId.equals=" + (invoiceId + 1));
    }


    @Test
    @Transactional
    public void getAllSupplierTransactionsByStatusIsEqualToSomething() throws Exception {
        // Initialize the database
        supplierTransactionsRepository.saveAndFlush(supplierTransactions);
        SupplierTransactionStatus status = SupplierTransactionStatusResourceIT.createEntity(em);
        em.persist(status);
        em.flush();
        supplierTransactions.setStatus(status);
        supplierTransactionsRepository.saveAndFlush(supplierTransactions);
        Long statusId = status.getId();

        // Get all the supplierTransactionsList where status equals to statusId
        defaultSupplierTransactionsShouldBeFound("statusId.equals=" + statusId);

        // Get all the supplierTransactionsList where status equals to statusId + 1
        defaultSupplierTransactionsShouldNotBeFound("statusId.equals=" + (statusId + 1));
    }
    ...
    public void updateSupplierTransactions() throws Exception {
        ...
        restSupplierTransactionsMockMvc.perform(put("/api/supplier-transactions").with(csrf())
            .contentType(MediaType.APPLICATION_JSON)
            .content(TestUtil.convertObjectToJsonBytes(supplierTransactionsDTO)))
            .andExpect(status().isOk());
    ...
    public void deleteSupplierTransactions() throws Exception {
        ...
        restSupplierTransactionsMockMvc.perform(delete("/api/supplier-transactions/{id}", supplierTransactions.getId()).with(csrf())
