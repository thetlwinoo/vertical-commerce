...
/**
 * Performance test for the Regions entity.
 */
class RegionsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Regions entity")
        ...
        .repeat(2) {
            ...
            .get("/services/vscommerce/api/regions")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new regions")
            .post("/services/vscommerce/api/regions")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_regions_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created regions")
                .get("/services/vscommerce${new_regions_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created regions")
            .delete("/services/vscommerce${new_regions_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
