...
/**
 * Performance test for the WishlistLines entity.
 */
class WishlistLinesGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the WishlistLines entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all wishlistLines")
            .get("/services/vscommerce/api/wishlist-lines")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new wishlistLines")
            .post("/services/vscommerce/api/wishlist-lines")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_wishlistLines_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created wishlistLines")
                .get("/services/vscommerce${new_wishlistLines_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created wishlistLines")
            .delete("/services/vscommerce${new_wishlistLines_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
