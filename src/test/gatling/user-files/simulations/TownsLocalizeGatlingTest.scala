...
/**
 * Performance test for the TownsLocalize entity.
 */
class TownsLocalizeGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the TownsLocalize entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all townsLocalizes")
            .get("/services/vscommerce/api/towns-localizes")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new townsLocalize")
            .post("/services/vscommerce/api/towns-localizes")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_townsLocalize_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created townsLocalize")
                .get("/services/vscommerce${new_townsLocalize_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created townsLocalize")
            .delete("/services/vscommerce${new_townsLocalize_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
