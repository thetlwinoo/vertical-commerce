...
/**
 * Performance test for the Reviews entity.
 */
class ReviewsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Reviews entity")
        ...
        .repeat(2) {
            ...
            .get("/services/vscommerce/api/reviews")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new reviews")
            .post("/services/vscommerce/api/reviews")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_reviews_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created reviews")
                .get("/services/vscommerce${new_reviews_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created reviews")
            .delete("/services/vscommerce${new_reviews_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
