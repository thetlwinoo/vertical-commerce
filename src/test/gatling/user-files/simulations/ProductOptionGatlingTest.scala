...
/**
 * Performance test for the ProductOption entity.
 */
class ProductOptionGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ProductOption entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all productOptions")
            .get("/services/vscommerce/api/product-options")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new productOption")
            .post("/services/vscommerce/api/product-options")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "value":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_productOption_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created productOption")
                .get("/services/vscommerce${new_productOption_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created productOption")
            .delete("/services/vscommerce${new_productOption_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
