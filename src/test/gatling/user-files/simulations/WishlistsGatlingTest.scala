...
/**
 * Performance test for the Wishlists entity.
 */
class WishlistsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Wishlists entity")
        ...
        .repeat(2) {
            ...
            .get("/services/vscommerce/api/wishlists")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new wishlists")
            .post("/services/vscommerce/api/wishlists")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_wishlists_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created wishlists")
                .get("/services/vscommerce${new_wishlists_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created wishlists")
            .delete("/services/vscommerce${new_wishlists_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
