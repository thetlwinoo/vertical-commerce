...
/**
 * Performance test for the CitiesCulture entity.
 */
class CitiesCultureGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the CitiesCulture entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all citiesCultures")
            .get("/services/vscommerce/api/cities-cultures")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new citiesCulture")
            .post("/services/vscommerce/api/cities-cultures")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_citiesCulture_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created citiesCulture")
                .get("/services/vscommerce${new_citiesCulture_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created citiesCulture")
            .delete("/services/vscommerce${new_citiesCulture_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
