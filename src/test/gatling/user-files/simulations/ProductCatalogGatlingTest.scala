...
/**
 * Performance test for the ProductCatalog entity.
 */
class ProductCatalogGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ProductCatalog entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all productCatalogs")
            .get("/services/vscommerce/api/product-catalogs")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new productCatalog")
            .post("/services/vscommerce/api/product-catalogs")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_productCatalog_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created productCatalog")
                .get("/services/vscommerce${new_productCatalog_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created productCatalog")
            .delete("/services/vscommerce${new_productCatalog_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
