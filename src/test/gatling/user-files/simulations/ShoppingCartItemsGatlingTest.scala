...
/**
 * Performance test for the ShoppingCartItems entity.
 */
class ShoppingCartItemsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ShoppingCartItems entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all shoppingCartItems")
            .get("/services/vscommerce/api/shopping-cart-items")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new shoppingCartItems")
            .post("/services/vscommerce/api/shopping-cart-items")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "quantity":"0"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_shoppingCartItems_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created shoppingCartItems")
                .get("/services/vscommerce${new_shoppingCartItems_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created shoppingCartItems")
            .delete("/services/vscommerce${new_shoppingCartItems_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
