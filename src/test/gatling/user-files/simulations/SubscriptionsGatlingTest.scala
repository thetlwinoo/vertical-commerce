...
/**
 * Performance test for the Subscriptions entity.
 */
class SubscriptionsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Subscriptions entity")
        ...
        .repeat(2) {
            ...
            .get("/services/vscommerce/api/subscriptions")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new subscriptions")
            .post("/services/vscommerce/api/subscriptions")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_subscriptions_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created subscriptions")
                .get("/services/vscommerce${new_subscriptions_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created subscriptions")
            .delete("/services/vscommerce${new_subscriptions_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
