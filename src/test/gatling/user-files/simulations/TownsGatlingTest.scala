...
/**
 * Performance test for the Towns entity.
 */
class TownsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Towns entity")
        ...
        .repeat(2) {
            ...
            .get("/services/vscommerce/api/towns")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new towns")
            .post("/services/vscommerce/api/towns")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_towns_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created towns")
                .get("/services/vscommerce${new_towns_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created towns")
            .delete("/services/vscommerce${new_towns_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
