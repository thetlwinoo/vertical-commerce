...
/**
 * Performance test for the Discounts entity.
 */
class DiscountsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Discounts entity")
        ...
        .repeat(2) {
            ...
            .get("/services/vscommerce/api/discounts")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new discounts")
            .post("/services/vscommerce/api/discounts")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_discounts_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created discounts")
                .get("/services/vscommerce${new_discounts_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created discounts")
            .delete("/services/vscommerce${new_discounts_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
