...
/**
 * Performance test for the TrackingEvent entity.
 */
class TrackingEventGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the TrackingEvent entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all trackingEvents")
            .get("/services/vscommerce/api/tracking-events")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new trackingEvent")
            .post("/services/vscommerce/api/tracking-events")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_trackingEvent_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created trackingEvent")
                .get("/services/vscommerce${new_trackingEvent_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created trackingEvent")
            .delete("/services/vscommerce${new_trackingEvent_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
