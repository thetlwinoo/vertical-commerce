...
/**
 * Performance test for the ShippingFeeChart entity.
 */
class ShippingFeeChartGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ShippingFeeChart entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all shippingFeeCharts")
            .get("/services/vscommerce/api/shipping-fee-charts")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new shippingFeeChart")
            .post("/services/vscommerce/api/shipping-fee-charts")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "sizeOfPercel":"SAMPLE_TEXT"
                , "minVolumeWeight":"0"
                , "maxVolumeWeight":"0"
                , "minActualWeight":"0"
                , "maxActualWeight":"0"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_shippingFeeChart_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created shippingFeeChart")
                .get("/services/vscommerce${new_shippingFeeChart_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created shippingFeeChart")
            .delete("/services/vscommerce${new_shippingFeeChart_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
