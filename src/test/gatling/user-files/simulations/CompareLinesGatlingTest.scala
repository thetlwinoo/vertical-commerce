...
/**
 * Performance test for the CompareLines entity.
 */
class CompareLinesGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the CompareLines entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all compareLines")
            .get("/services/vscommerce/api/compare-lines")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new compareLines")
            .post("/services/vscommerce/api/compare-lines")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_compareLines_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created compareLines")
                .get("/services/vscommerce${new_compareLines_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created compareLines")
            .delete("/services/vscommerce${new_compareLines_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
