...
/**
 * Performance test for the Receipts entity.
 */
class ReceiptsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Receipts entity")
        ...
        .repeat(2) {
            ...
            .get("/services/vscommerce/api/receipts")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new receipts")
            .post("/services/vscommerce/api/receipts")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_receipts_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created receipts")
                .get("/services/vscommerce${new_receipts_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created receipts")
            .delete("/services/vscommerce${new_receipts_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
