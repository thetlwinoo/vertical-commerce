...
/**
 * Performance test for the DiscountTypes entity.
 */
class DiscountTypesGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the DiscountTypes entity")
        ...
        .repeat(2) {
            ...
            .get("/services/vscommerce/api/discount-types")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new discountTypes")
            .post("/services/vscommerce/api/discount-types")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_discountTypes_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created discountTypes")
                .get("/services/vscommerce${new_discountTypes_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created discountTypes")
            .delete("/services/vscommerce${new_discountTypes_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
