...
/**
 * Performance test for the TaxClass entity.
 */
class TaxClassGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the TaxClass entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all taxClasses")
            .get("/services/vscommerce/api/tax-classes")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new taxClass")
            .post("/services/vscommerce/api/tax-classes")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "code":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_taxClass_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created taxClass")
                .get("/services/vscommerce${new_taxClass_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created taxClass")
            .delete("/services/vscommerce${new_taxClass_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
