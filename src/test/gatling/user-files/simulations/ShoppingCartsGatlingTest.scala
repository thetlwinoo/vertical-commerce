...
/**
 * Performance test for the ShoppingCarts entity.
 */
class ShoppingCartsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ShoppingCarts entity")
        ...
        .repeat(2) {
            ...
            .get("/services/vscommerce/api/shopping-carts")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new shoppingCarts")
            .post("/services/vscommerce/api/shopping-carts")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "totalPrice":"0"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_shoppingCarts_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created shoppingCarts")
                .get("/services/vscommerce${new_shoppingCarts_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created shoppingCarts")
            .delete("/services/vscommerce${new_shoppingCarts_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
