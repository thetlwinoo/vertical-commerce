...
/**
 * Performance test for the WarrantyTypes entity.
 */
class WarrantyTypesGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the WarrantyTypes entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all warrantyTypes")
            .get("/services/vscommerce/api/warranty-types")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new warrantyTypes")
            .post("/services/vscommerce/api/warranty-types")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_warrantyTypes_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created warrantyTypes")
                .get("/services/vscommerce${new_warrantyTypes_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created warrantyTypes")
            .delete("/services/vscommerce${new_warrantyTypes_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
