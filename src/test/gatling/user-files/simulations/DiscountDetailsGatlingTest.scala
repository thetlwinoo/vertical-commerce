...
/**
 * Performance test for the DiscountDetails entity.
 */
class DiscountDetailsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the DiscountDetails entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all discountDetails")
            .get("/services/vscommerce/api/discount-details")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new discountDetails")
            .post("/services/vscommerce/api/discount-details")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "amount":"0"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_discountDetails_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created discountDetails")
                .get("/services/vscommerce${new_discountDetails_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created discountDetails")
            .delete("/services/vscommerce${new_discountDetails_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
