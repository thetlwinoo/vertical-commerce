...
/**
 * Performance test for the Cards entity.
 */
class CardsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Cards entity")
        ...
        .repeat(2) {
            ...
            .get("/services/vscommerce/api/cards")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new cards")
            .post("/services/vscommerce/api/cards")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "number":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_cards_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created cards")
                .get("/services/vscommerce${new_cards_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created cards")
            .delete("/services/vscommerce${new_cards_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
