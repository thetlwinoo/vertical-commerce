...
/**
 * Performance test for the TownsCulture entity.
 */
class TownsCultureGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the TownsCulture entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all townsCultures")
            .get("/services/vscommerce/api/towns-cultures")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new townsCulture")
            .post("/services/vscommerce/api/towns-cultures")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_townsCulture_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created townsCulture")
                .get("/services/vscommerce${new_townsCulture_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created townsCulture")
            .delete("/services/vscommerce${new_townsCulture_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
