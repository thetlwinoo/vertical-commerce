...
/**
 * Performance test for the CustomerPaymentBankTransfer entity.
 */
class CustomerPaymentBankTransferGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the CustomerPaymentBankTransfer entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all customerPaymentBankTransfers")
            .get("/services/vscommerce/api/customer-payment-bank-transfers")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new customerPaymentBankTransfer")
            .post("/services/vscommerce/api/customer-payment-bank-transfers")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_customerPaymentBankTransfer_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created customerPaymentBankTransfer")
                .get("/services/vscommerce${new_customerPaymentBankTransfer_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created customerPaymentBankTransfer")
            .delete("/services/vscommerce${new_customerPaymentBankTransfer_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
