...
/**
 * Performance test for the ProductSet entity.
 */
class ProductSetGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ProductSet entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all productSets")
            .get("/services/vscommerce/api/product-sets")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new productSet")
            .post("/services/vscommerce/api/product-sets")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
                , "noOfPerson":"0"
                , "isExclusive":null
                , "modifinedDate":"2020-01-01T00:00:00.000Z"
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_productSet_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created productSet")
                .get("/services/vscommerce${new_productSet_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created productSet")
            .delete("/services/vscommerce${new_productSet_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
