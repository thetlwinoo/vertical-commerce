...
/**
 * Performance test for the ProductChoice entity.
 */
class ProductChoiceGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ProductChoice entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all productChoices")
            .get("/services/vscommerce/api/product-choices")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new productChoice")
            .post("/services/vscommerce/api/product-choices")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "isMultiply":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_productChoice_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created productChoice")
                .get("/services/vscommerce${new_productChoice_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created productChoice")
            .delete("/services/vscommerce${new_productChoice_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
