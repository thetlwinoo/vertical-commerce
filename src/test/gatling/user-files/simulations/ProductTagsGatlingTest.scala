...
/**
 * Performance test for the ProductTags entity.
 */
class ProductTagsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ProductTags entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all productTags")
            .get("/services/vscommerce/api/product-tags")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new productTags")
            .post("/services/vscommerce/api/product-tags")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_productTags_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created productTags")
                .get("/services/vscommerce${new_productTags_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created productTags")
            .delete("/services/vscommerce${new_productTags_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
