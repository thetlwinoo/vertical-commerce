...
/**
 * Performance test for the CardTypes entity.
 */
class CardTypesGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the CardTypes entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all cardTypes")
            .get("/services/vscommerce/api/card-types")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new cardTypes")
            .post("/services/vscommerce/api/card-types")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_cardTypes_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created cardTypes")
                .get("/services/vscommerce${new_cardTypes_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created cardTypes")
            .delete("/services/vscommerce${new_cardTypes_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
