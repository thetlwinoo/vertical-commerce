...
/**
 * Performance test for the ReviewLines entity.
 */
class ReviewLinesGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ReviewLines entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all reviewLines")
            .get("/services/vscommerce/api/review-lines")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new reviewLines")
            .post("/services/vscommerce/api/review-lines")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "stockItemRating":"0"
                , "stockItemReview":null
                , "customerReviewedOn":"2020-01-01T00:00:00.000Z"
                , "completedReviewLine":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_reviewLines_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created reviewLines")
                .get("/services/vscommerce${new_reviewLines_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created reviewLines")
            .delete("/services/vscommerce${new_reviewLines_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
