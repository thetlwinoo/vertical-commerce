...
/**
 * Performance test for the Compares entity.
 */
class ComparesGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Compares entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all compares")
            .get("/services/vscommerce/api/compares")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new compares")
            .post("/services/vscommerce/api/compares")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_compares_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created compares")
                .get("/services/vscommerce${new_compares_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created compares")
            .delete("/services/vscommerce${new_compares_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
