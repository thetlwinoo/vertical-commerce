...
/**
 * Performance test for the CreditCardType entity.
 */
class CreditCardTypeGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the CreditCardType entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all creditCardTypes")
            .get("/services/vscommerce/api/credit-card-types")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new creditCardType")
            .post("/services/vscommerce/api/credit-card-types")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_creditCardType_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created creditCardType")
                .get("/services/vscommerce${new_creditCardType_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created creditCardType")
            .delete("/services/vscommerce${new_creditCardType_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
