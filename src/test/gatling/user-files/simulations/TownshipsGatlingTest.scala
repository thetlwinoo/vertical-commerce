...
/**
 * Performance test for the Townships entity.
 */
class TownshipsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Townships entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all townships")
            .get("/services/vscommerce/api/townships")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new townships")
            .post("/services/vscommerce/api/townships")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_townships_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created townships")
                .get("/services/vscommerce${new_townships_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created townships")
            .delete("/services/vscommerce${new_townships_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
