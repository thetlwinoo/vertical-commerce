...
/**
 * Performance test for the OrderPackages entity.
 */
class OrderPackagesGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the OrderPackages entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all orderPackages")
            .get("/services/vscommerce/api/order-packages")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new orderPackages")
            .post("/services/vscommerce/api/order-packages")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                ...
                , "packageShippingFee":"0"
                , "packageShippingFeeDiscount":"0"
                , "packagePrice":"0"
                , "packageSubTotal":"0"
                , "packageTaxAmount":"0"
                , "packageVoucherDiscount":"0"
                , "packagePromotionDiscount":"0"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_orderPackages_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created orderPackages")
                .get("/services/vscommerce${new_orderPackages_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created orderPackages")
            .delete("/services/vscommerce${new_orderPackages_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
