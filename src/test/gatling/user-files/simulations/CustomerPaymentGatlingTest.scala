...
/**
 * Performance test for the CustomerPayment entity.
 */
class CustomerPaymentGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the CustomerPayment entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all customerPayments")
            .get("/services/vscommerce/api/customer-payments")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new customerPayment")
            .post("/services/vscommerce/api/customer-payments")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_customerPayment_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created customerPayment")
                .get("/services/vscommerce${new_customerPayment_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created customerPayment")
            .delete("/services/vscommerce${new_customerPayment_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
