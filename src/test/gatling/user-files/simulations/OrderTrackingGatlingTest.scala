...
/**
 * Performance test for the OrderTracking entity.
 */
class OrderTrackingGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the OrderTracking entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all orderTrackings")
            .get("/services/vscommerce/api/order-trackings")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new orderTracking")
            .post("/services/vscommerce/api/order-trackings")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "carrierTrackingNumber":"SAMPLE_TEXT"
                , "eventDetails":null
                , "eventDate":"2020-01-01T00:00:00.000Z"
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_orderTracking_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created orderTracking")
                .get("/services/vscommerce${new_orderTracking_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created orderTracking")
            .delete("/services/vscommerce${new_orderTracking_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
