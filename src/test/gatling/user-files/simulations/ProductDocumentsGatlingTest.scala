...
/**
 * Performance test for the ProductDocuments entity.
 */
class ProductDocumentsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ProductDocuments entity")
        ...
        .repeat(2) {
            ...
            .get("/services/vscommerce/api/product-documents")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new productDocuments")
            .post("/services/vscommerce/api/product-documents")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_productDocuments_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created productDocuments")
                .get("/services/vscommerce${new_productDocuments_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created productDocuments")
            .delete("/services/vscommerce${new_productDocuments_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
