...
/**
 * Performance test for the Logistics entity.
 */
class LogisticsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Logistics entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all logistics")
            .get("/services/vscommerce/api/logistics")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new logistics")
            .post("/services/vscommerce/api/logistics")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_logistics_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created logistics")
                .get("/services/vscommerce${new_logistics_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created logistics")
            .delete("/services/vscommerce${new_logistics_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
